...
/*
@ResponseBody + Controller 와 같은 형태로 Rest API를 만들기 위한 컨트롤러에서 주로
사용된다. 내부 메서드에서 반환되는 값은 즉시 웹 브라우저에 출력된다. (일반적인 컨트롤러에서는 
반환값이 View의 경로가 된다.
 */
@RestController
public class MyRestController {
	...
	//JSON 객체
	/*
	Map은 Key와 Value를 가진 구조이므로 JSON 객체와 동일하다. 따라서 Map을 반환하면
	JSON 객체 형태로 출력된다.
	 */
	...
	/*
	JSON 배열
	: 배열은 List와 동일한 구조를 가진다. 특정 Key 없이 인덱스로 접근한다.
	 */
	...
	/*
	JSON 객체이지만 Value로 배열을 포함하는 형태로 실무에서 가장 많이 사용된다.
	 */
	@GetMapping("/restApi03.do")
	public Map<String, Object> restApi03() {
		//데이터는 객체이므로 Map을 생성한다.
		Map<String, Object> maps = new HashMap<>();
	...
	//JSON 배열이지만 객체를 인자로 가진다. 게시판의 목록에서 많이 사용된다.
